<?php
  ...
  // isset — Определяет, была ли установлена переменная значением отличным от NULL. isset() вернет FALSE, если проверяемая переменная имеет значение NULL.

  // Typically, this would be a call to a database
  // Функция генерирует посты, которые будут подгружаться кнопкой load more! (иммитация базы данных)
  function find_blog_posts($page) {
    $first_post = 101;
    $per_page = 3; // подгружаем по 3 поста
  ...
  // Для явного преобразования в integer, используют приведение (int) или (integer). 
